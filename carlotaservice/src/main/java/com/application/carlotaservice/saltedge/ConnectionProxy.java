...
import com.application.carlotaservice.saltedge.dto.*;
...
public class ConnectionProxy {
  ...
  public void showConnections(String connection) {
    ...
    ResponseEntity<DataConnectionDTO> response =
        restTemplate.exchange(
            CONNECTION_URL + connection, HttpMethod.GET, entity, DataConnectionDTO.class);
  ...
  public void removeConnection(String connection) {
    ...
    ResponseEntity<DataConnectionDTO> response =
        restTemplate.exchange(
            CONNECTION_URL + connection, HttpMethod.DELETE, entity, DataConnectionDTO.class);
  ...
  public void showConnectionList(String customerID) {
    ...
    ResponseEntity<DataConnectionListDTO> response =
        restTemplate.exchange(
            "https://www.saltedge.com/api/v5/connections?customer_id=" + customerID,
            HttpMethod.GET,
            entity,
            DataConnectionListDTO.class);
  ...
  public DataConnectionDTO createConnection(
      String customerID, String providerCode, String login, String password) {
    setHttpHeaders();
    ...
    AttemptDTO attemptDTO =
        AttemptDTO.builder().fetch_scopes(Arrays.asList("accounts", "transactions")).build();

    CredentialsDTO credentialasDTO =
        CredentialsDTO.builder().login(login).password(password).build();
